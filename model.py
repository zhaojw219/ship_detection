...
from keras_contrib.layers import InstanceNormalization,GroupNormalization
...
def identity_block(input_tensor, kernel_size, filters, stage, block,use_bias=True, train_bn=True):
    ...
    x = LeakyReLU(alpha=0.05, name='res' + str(stage) + block + '_out')(x)
    return x
...
def rpn_graph(feature_map, anchors_per_location, anchor_stride):
    ...
    shared = KL.Conv2D(512, (3, 3), padding='same',
                       strides=anchor_stride,
                       name='rpn_conv_shared')(feature_map)

    shared = LeakyReLU(alpha=0.05)(shared)
...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
    ...
    x = KL.TimeDistributed(KL.MaxPooling2D(pool_size=(2,2),strides=2,border_mode='valid',dim_ordering='tf'),
                           name="max_pooling")(x)
    ...
    x = KL.TimeDistributed(BatchNorm(),name='mrcnn_mask_bn8')(x,training=train_bn)
    # x = KL.Activation('relu')(x)
    x = LeakyReLU(alpha=0.05)(x)

    x = KL.TimeDistributed(KL.Conv2D(64, (3, 3), padding="same"),
                           name="mrcnn_mask_conv9")(x)
    x = KL.TimeDistributed(BatchNorm(),name='mrcnn_mask_bn9')(x,training=train_bn)
    # x = KL.Activation('relu')(x)
    x = LeakyReLU(alpha=0.05)(x)

    x = KL.TimeDistributed(KL.Conv2D(num_classes, (1, 1), strides=1, activation="sigmoid"),
                           name="mrcnn_mask_conv10")(x)
    return x
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            # Losses
            rpn_class_loss = KL.Lambda(lambda x: rpn_class_loss_graph(config, *x), name="rpn_class_loss")(
